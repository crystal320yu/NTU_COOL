...
## 1.ATP Top3選手在1968~2018年間"硬地及草地"比賽獲勝率


<font size="4">Download file、解壓縮、列出目錄/文件夾的檔案 </font><br />  
```{r }
...
<font size="4">#讀取資料，設定cols</font><br />  
```{r , echo= FALSE}
...
<font size="4">運用do.call & lapply處理資料</font><br />
```{r }
...
<font size="4">搜尋網路上ATP排名前100名的選手</font><br />
```{r }
library(xml2)
library(rvest)
...
<font size="4">只求前3名的選手名</font><br />
```{r }
top3.name <- top100.players[1:3]
```


<font size="4">從data frame中找出跟網站上名字最相近的字，由於web上目前排名14的阿根挺籍選手Diego Schwartzman，在CSV中顯示為Diego Sebastian Schwartzman。</font><br />
<font size="4">藉由amatch找出相近字串這個方法會一直跑出另一個Diego Sxxxxxx的選手，因此我用手動調整csv檔，將所有的Diego Sebastian Schwartzman replace by Diego Schwartzman。</font><br />
```{r }
library(stringdist)
...
<font size="4">運用tibble可以更方便使用dplyr</font><br />
<font size="4">讀取前3名</font><br />
```{r }
...
<font size="4">做出ATP Top3選手在1968~2018年間"硬地及草地"比賽獲勝率圖表</font><br />
<font size="4">結論由圖可了解，對Djokovic& Nadal來說，clay(硬地)材質的環境比較容易獲勝；Federer在草地上發揮得比較平均。</font><br />
```{r }
library(ggplot2)
...
##2.列出Novak Djokovic在2018年中不同比賽場地材質(Clay, Hard,Grass,Carpet)獲勝機率比較分析


<font size="4">由於我最喜歡的選手為"Novak Djokovic"，因此決定再對他個人進行分析</font><br />
<font size="4">使用subset，先選出2018中比賽贏&輸時的比賽場地材質(surface)</font><br />
```{r }
...
<font size="4">將輸及贏的場地用rbind結合，算出不同場地材質的比賽總次數</font><br />
<font size="4">依照不同場地材質(贏次數 / 總次數) = 獲勝機率</font><br />
```{r }
...
<font size="4">做圖，將顏色改為橘色填滿，外框用黑色，調整柱狀圖寬度</font><br />
<font size="4">列出Novak Djokovic在不同比賽場地材質獲勝機率</font><br />
<font size="4">結論由圖可了解，對Djokovic& Nadal來說，Hard(硬地)材質的環境比較容易獲勝</font><br />
```{r}
library(ggrepel)
...
<font size="4">讀取csv檔</font><br />
```{r }
Tennis2018.source <-read.csv("file:///C:/Users/user/Desktop/NTU_COOL/week_3/tennis_atp-master/atp_matches_2018.csv")
```


<font size="4">運用sebset選出winner_name, loser_name, winner_ioc, loser_ioc</font><br />
<font size="4">將col名字改為一樣，用rbind連接，並使用unique</font><br />
```{r }
...
<font size="4">由小到大排序後，選出人數>8的國家</font><br />
```{r }
...
<font size="4">做圖，藍色長條圖</font><br />
<font size="4">運用geom_text，數據以白色呈現列出選手數量</font><br />
```{r}
